/**
 * @file        common.js
 * @brief       전역 공통 스크립트. www 강제 이동, 폼 필드 검증 및 공통 유틸
...
function set_comment_token(f)
{
    ...
    $.ajax({
        url: g5_bbs_url+"/ajax.comment_token",
        type: "GET",
        dataType: "json",
        async: false,
        cache: false,
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        await injectPartial("#app-footer", `/sh_page/footer.html`);
    } catch (e) {
      console.error(e);
    }
});  

async function injectMenu(selector, url) {
    const host = document.querySelector(selector);
    if (!host) return;
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        await injectMenu("#app-menu", `/sh_page/menu.html`);
    } catch (e) {
      console.error(e);
    }
});  

async function injectSubMenu(selector, url) {
    const host = document.querySelector(selector);
    if (!host) return;
...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        await injectSubMenu("#app-sub-menu", `/sh_page/subMenu.html`);
    } catch (e) {
      console.error(e);
    }
});  